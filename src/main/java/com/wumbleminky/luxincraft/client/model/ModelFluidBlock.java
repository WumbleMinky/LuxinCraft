package com.wumbleminky.luxincraft.client.model;
...
public class ModelFluidBlock implements ISmartBlockModel{
	
	static final float RENDER_OFFSET = 0.001F;

    public ModelFluidBlock(){
	...
	public IBakedModel handleBlockState(IBlockState state){
		...
		float flow = extState.getValue(BlockFluid.FLOW_DIRECTION);
		
		return getModel((BlockFluid) state.getBlock(), Arrays.asList(heightSE, heightSW, heightNW, heightNE, flow));
	}
	
	private SimpleBakedModel getModel(BlockFluid block, List<Float> param){
	...
	private SimpleBakedModel buildModel(BlockFluid block, List<Float> param){
